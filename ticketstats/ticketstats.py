...
class TicketStats(commands.Cog):
    ...
    async def dm_status(self):
        if self.bot.config["dm_disabled"] == DMDisabled.ALL_THREADS:
            ...
            if self.activity:
                await self.bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=f"No New Tickets"), status=discord.Status.dnd)
            return "New Tickets Disabled"
        elif self.tickets_open > self.tickets_backlog:
            if self.activity:
                await self.bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=f"Watching Delayed Responses"), status=discord.Status.idle)
            return "Backlogged"
        else:
            if self.activity:
                await self.bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=f"Open a ticket for support ☎️"), status=discord.Status.online)
            return "Normal"
